...
def alert(msg):
    xbmc.executebuiltin('XBMC.Notification(Info:,'+msg+' (DailyMotion)!,5000)')
...
def playLiveVideo(id):
    ...
    if content.find('"statusCode":410') > 0 or content.find('"statusCode":403') > 0:
        ...
        if matchFullHD and maxVideoQuality == "1080p":
            ...
            hdsurl = urllib.unquote_plus(matchhds[0]).replace("\\", "")
            print hdsurl        
        elif matchLD:
            url = urllib.unquote_plus(matchSD2[0]).replace("\\", "")        

        if hdsurl:
            alert('F4M stream detected')
            #url = urllib2.Request(hdsurl) # f4m playable link and need to pass to f4mtester
            #req = urllib2.Request(hdsurl)
            print 'f4m link is found calling pluginf4mtester "You need f4mtester plugin by Shani\r\n https://bitbucket.org/shani-08/shanixbmcwork/src'    
            from F4mProxy import f4mProxyHelper
            import requests2 as requests
            req = requests.get(hdsurl, allow_redirects=False)
            url = req.headers.get('Location')
            print ('found location: %s' %url)
